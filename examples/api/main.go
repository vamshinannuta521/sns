...
func main() {
	...
	flag.StringVar(&kafkaBrokerUrl, "kafka-brokers", "10.46.143.17:9092", "Kafka brokers in comma separated value")
	...
	// connect to kafka
	kafkaProducer, err := kafka_utils.Configure(strings.Split(kafkaBrokerUrl, ","), kafkaClientId, kafkaTopic)
	if err != nil {
...
func postDataToKafka(ctx *gin.Context) {
	...
	err = kafka_utils.Push(parent, nil, formInBytes)
	if err != nil {
		ctx.JSON(http.StatusUnprocessableEntity, map[string]interface{}{
			"error": map[string]interface{}{
